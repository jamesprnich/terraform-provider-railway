...
variable "project_name" {
  ...
  default     = "AAA-provctest-t3-2-e2e"
}
...
resource "railway_service" "postgres" {
  ...
  # Inline volume — exercises the readback retry path that the v0.11.1 fix
  # touches. Was standalone `railway_volume` in the upstream example; swapped
  # for the E2E test so the fix is actually stressed end-to-end.
  volume = {
...
# Standalone volume on the app service — kept alongside the inline volume
# above so this E2E exercises BOTH inline and standalone code paths in the
# same apply. Both must succeed for the E2E to pass; if the fix broke either
# path in isolation this catches it.
resource "railway_volume" "app_cache" {
  name           = "dev-app-cache"
  project_id     = railway_project.demo.id
