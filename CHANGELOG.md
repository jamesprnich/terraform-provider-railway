...
* **`railway_volume` Create could taint a fresh volume on a transient rename failure, and combined with `prevent_destroy` on a data volume, wedge unrelated applies.** Railway's `volumeCreate` mutation has no `name` field (confirmed against `schema.graphql` — this is an upstream API limitation, not a codegen gap), so a named `railway_volume` is created with the server-default name and then renamed via a separate `updateVolume` call. That rename call had no retry wrapping, unlike the create and post-create readback next to it (both already hardened in v0.11.1). A transient error on the rename — Railway's per-mutation throttle, or the same eventual-consistency window that affects the readback — left the volume tainted, since state is (correctly) persisted immediately after `createVolume` succeeds. Every subsequent apply then planned to replace the tainted volume; `prevent_destroy` on a protected data volume turned that into a hard `Error: Instance cannot be destroyed`, blocking any apply touching unrelated resources until someone manually intervened.

  Fixed by wrapping the rename call in `Create` — and the equivalent `updateVolume`/`updateVolumeInstance` calls in `Update` — in the same retry envelope as the existing create/readback calls (`retryUpdateContext` in `internal/provider/helpers.go`). This closes the gap for the transient case, which is the common case in practice.

  For the rare case where the rename genuinely never succeeds even after retries: the volume is deliberately left as a visible, replaceable taint rather than rolled back (deleted) or hidden. We looked at how the best-maintained Terraform providers handle this same shape of problem — a Create split across multiple API calls, where the first call has already created a real remote object before a later call can fail. AWS (`rds/instance.go`, `ec2/ebs_volume.go`, `efs/file_system.go`, `dynamodb/table.go`), Google (`resource_sql_database_instance.go`, which has its own multi-step post-create follow-up calls), and Azure (`postgresql_flexible_server_resource.go`) all persist state immediately after the initial create and never roll back on a later step's failure — they retry hard and, on exhaustion, just return a plain error. None of them attempt a rollback-delete. A rollback would be a genuinely novel pattern for this provider, and it trades a visible, recoverable taint for a worse failure mode: the same transient conditions that broke the rename could just as easily break the cleanup delete, silently orphaning a real, billed Railway volume with no Terraform record of it at all. `RequiresReplace` on `name`/`mount_path` was also considered and rejected for the same reason — Railway's API can rename a volume and move a mount path in place; forcing replacement of a `prevent_destroy` data volume on a routine rename is worse than the bug it would "fix."

  Do not treat a taint from exhausted rename retries as something the provider should have prevented outright — it's the same accepted trade-off every major provider makes for this resource shape. The operational backstop is a CI plan-gate that fails any plan proposing to destroy/replace a `prevent_destroy` resource, catching this before a real apply — not a provider-side rollback.

### Tests

* **`TestVolumeResource_createRenameRetryRecoversFromTransientFailure`** — a transient rename failure (2-3 attempts) during Create recovers automatically; the volume ends up with the correct final name and no operator-visible error.
* **`TestVolumeResource_createRenameRetryExhausted_leavesVisibleTaintNotOrphan`** — when the rename genuinely never succeeds, Create surfaces a clear error, and — the critical assertion — the volume it actually created remains visible in state as a replaceable taint (`plancheck.ExpectResourceAction(..., ResourceActionReplace)` on the next plan), not silently dropped. Uses a stateful mock server (`newTaintThenReplaceMockServer`) so this is empirically verified against OpenTofu's actual refresh/taint/replace behavior, not just asserted.
* **`TestVolumeResource_updateRetryRenamesInPlaceUnderPreventDestroy`** — renaming and moving the mount path on an existing `prevent_destroy`-protected volume both recover from transient failures via the same retry wrapping and apply as in-place updates (`plancheck.ExpectResourceAction(..., ResourceActionUpdate)`). Since OpenTofu refuses to destroy a `prevent_destroy` resource, a clean successful apply here is itself direct proof the plan action was never Replace.
